...
int main(const int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        for (int r = 1; r < pcount; r++) {
            MPI_Send(&arr,  sizeof(arr) / sizeof(arr[0]), MPI_INT, r, tag, MPI_COMM_WORLD);
        }
        for (int r = 1; r < pcount; r++) {
            int i = 0;
            MPI_Recv(&i, 1, MPI_INT, r, tag, MPI_COMM_WORLD, &status);
            printf("Intero da rank %d: %d\n", r, i);
        }
        for (int r = 1; r < pcount; r++) {
            int temp_arr[] = {arr[r -1], arr[r], arr[r + 1]};
            MPI_Send(&arr,  3, MPI_INT, r, tag, MPI_COMM_WORLD);
        }
    } else {
