...
for s in "${STD_SIZES[@]}"; do
    ...
    for p in "${PROCS[@]}"; do
        ...
        mpiexec -n $p $PINNING $EXE -M $s -N $s -G 100 -FN $fn 2>&1 | tee -a $LOG_STD
    done
done
...
for p in "${PROCS[@]}"; do
    ...
    mpiexec -n $p $PINNING $EXE -M 20000 -N 20000 -G 100 -FN "matrix_20000x20000_seed1234.bin" 2>&1 | tee -a $LOG_STD
    
    echo "[BIG] Size: 50000x50000 | Processi: $p"
    echo -e "\n>>> Test NORMALE: 50000x50000 con $p processi <<<" >> $LOG_STD
    mpiexec -n $p $PINNING $EXE -M 50000 -N 50000 -G 20 -FN "matrix_50000x50000_seed1234.bin" 2>&1 | tee -a $LOG_STD
done
...
for p in "${VTUNE_PROCS[@]}"; do
    for vtype in "${VTUNE_TYPES[@]}"; do
        ...
        mpiexec -n $p $PINNING vtune -collect $vtype -data-limit=2000 -result-dir $VTUNE_DIR -- $EXE -M 20000 -N 20000 -G 100 -FN "matrix_20000x20000_seed1234.bin" 2>&1 | tee -a $LOG_VTUNE
    done
done
...
for p in "${EXT_PROCS[@]}"; do
    ...
    mpiexec -n $p $PINNING $EXE -M 100000 -N 50000 -G 5 -FN "matrix_100000x50000_seed1234.bin" 2>&1 | tee -a $LOG_STD
    
    echo "[EXTREME] 100kx100k | Processi: $p"
    echo -e "\n>>> Test EXTREME: 100000x100000 con $p processi <<<" >> $LOG_STD
    mpiexec -n $p $PINNING $EXE -M 100000 -N 100000 -G 5 -FN "matrix_100000x100000_seed1234.bin" 2>&1 | tee -a $LOG_STD
done
