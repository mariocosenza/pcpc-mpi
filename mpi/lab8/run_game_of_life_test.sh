...
for p in "${VTUNE_PROCS[@]}"; do
    for vtype in "${VTUNE_TYPES[@]}"; do
        ...
        mpiexec -n $p -hostfile $HOSTS $PINNING vtune -collect $vtype -data-limit=2000 -result-dir $VTUNE_DIR -- $EXE -M 20000 -N 20000 -G 100 -FN "matrix_20000x20000_seed1234_pattern0.bin" 2>&1 | tee -a $LOG_VTUNE
    done
done
